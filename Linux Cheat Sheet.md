...
1. **who**: Getting information about currently logged-in user as follows:
   ...
   ```bash
   $ who
    rhedayati console  May 31 13:58
    rhedayati ttys000  Jun 18 10:09
   ```

2. **whoami**: Displaying the system’s username
...
3. **id:** Displaying the user identification (the real and effective users and group IDs) information

   ```bash
   $ id
   uid=1000(sj) gid=1000(sj) groups=1000(sj),4(adm),24(cdrom),27(sudo),30(dip),46(plugdev),120(lpadmin),131(lxd),132(sambashare)
   ```
4. **groups:** Displaying all the groups for which the user belongs to.
...
5. **finger:**  Checking the information of any currently logged in users. i.e, It displays users login time, tty (name), idle time, home directory, shell name etc.

   ```bash
   $ finger
    Login    Name                 TTY  Idle  Login  Time   Office  Phone
    rhedayati Reza Hedayati       *con   18d  31 May 13:58
    rhedayati Reza Hedayati        s00        Sat    10:09
   ```

6. **users:** Displaying usernames of all users currently logged on the system.
...
7. **grep:** It  is a powerful pattern searching tool. One of the use is to find information about a specific user from the system accounts file: /etc/passwd.
